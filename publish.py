...
from pylib.config import (
    METADATA_FILE, GDRIVE_BLOG_FOLDER, TAG_FILTER_SCRIPT,
    CONTENT_TYPE_WORDS, CONTENT_TYPE_PROJECTS, CONTENT_TYPE_PAGES,
    WORDS_ARCHIVE_FILE
)
...
def compress_image(save_path, max_width=1024):
    """
    Compress and resize an image using macOS sips.
    Resizes to max_width if wider, and applies JPEG compression.
    """
    try:
        # Get current width
...
def convert_to_html(document, metadata, content_start_index=0, content_type='words', slug=''):
    ...
    # Browser title: prefer metadata browser-title, then h1 from doc, then doc name
    browser_title = metadata.get('browser-title') or metadata.get('Browser-title') or h1_title or doc_title
    ...
    canonical_path = f'/{slug}/' if content_type == 'pages' else f'/{content_type}/{slug}/'
    ...
    # Navigation (no active page for posts)
    html_parts.append(site_nav())
    html_parts.append('')
        ...
        if 'paragraph' in element:
            ...
            if html_content.strip():
                ...
                if '[ENDSNIP]' in html_content:
                    # Flush any pending image first to preserve document order
...
def git_commit_and_push(filename, doc_title, include_archive=False, include_homepage=False, include_metadata=False, include_sitemap=False, images=None):
    ...
    try:
        ...
        if include_archive:
            subprocess.run(['git', 'add', WORDS_ARCHIVE_FILE], check=True)
